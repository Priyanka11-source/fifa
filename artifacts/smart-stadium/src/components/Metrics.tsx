...
export default function Metrics() {
  ...
  const METRICS = [
    ...
    },
    {
      ...
      value: state
        ? String(state.transport.filter((t) => t.status === "normal").length)
        : "—",
  ...
  const TrendIcon = ({ trend }: { trend: string }) => {
    ...
    if (trend === "down")
      return <TrendingDown className="w-3.5 h-3.5 text-secondary-foreground" />;
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-5">
              <div
                className={`w-8 h-8 rounded-sm border flex items-center justify-center ${scheme.iconBg}`}
              >
                <Icon className={`w-4 h-4 ${scheme.text}`} />
              </div>
              <span
                className={`font-mono text-[9px] uppercase tracking-[0.2em] ${scheme.label}`}
              >
                {metric.label}
              </span>
            </div>
            ...
            <div className="flex items-baseline gap-2 mb-1">
              <span
                className={`text-5xl font-serif font-bold transition-all duration-300 ${scheme.text}`}
              >
                {metric.value}
              </span>
              <span className={`font-mono text-sm ${scheme.unit}`}>
                {metric.unit}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 mt-auto pt-3 border-t border-white/5">
              ...
              <span className="text-xs font-light text-foreground/55">
                {metric.sub}
              </span>
            </div>
          </motion.div>
