...
export default function AudienceExperience() {
  ...
  async function captureMockSnapshot() {
    setScanLog((prev) => [
      ...prev,
      "Capturing simulated frame...",
      "⟳ Analyzing with Gemini AI Vision...",
    ]);
    setIsLoading(true);
    ...
    try {
      ...
      if (data.success) {
        ...
        setIsSuccess(false);
        setScanLog((prev) => [...prev, `✗ Access Denied: ${data.message}`]);
        toast({
          title: "Access Denied",
          description: data.message,
          variant: "destructive",
    ...
    <div className="w-full flex flex-col gap-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[10px] font-mono text-neon-pink uppercase tracking-[0.2em] font-semibold">
            Interactive Fan Utilities
          </span>
        </div>
        <h3 className="font-serif text-3xl text-foreground">
          Smart Audience Experience
        </h3>
        <p className="text-xs text-foreground/60 mt-1.5 font-light leading-relaxed max-w-2xl">
          Access automated stadium services powered by Gemini AI. Scan matchday
          passes with your camera, order concourse food directly, and monitor
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-sm font-semibold uppercase tracking-wider font-mono text-foreground">
                Gate Entry Scanner
              </h4>
            </div>
            <span
              className={`text-[9px] font-mono px-2 py-0.5 border rounded flex items-center gap-1.5 ${
                isScanning
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${isScanning ? "bg-neon-pink animate-pulse" : "bg-foreground/20"}`}
              />
          ...
          <div className="relative aspect-video bg-black/90 border border-white/10 rounded-sm overflow-hidden flex flex-col items-center justify-center">
            {isScanning ? (
              <>
                ...
                {useSimulator && (
                  <SimulatedScannerCanvas isActive={isStreaming} />
                )}
                ...
                {isLoading && (
                  ...
                  >
                    ...
                    <span className="text-[9px] font-mono uppercase tracking-widest text-neon-pink bg-black/60 px-3 py-1 rounded border border-neon-pink/15">
                ...
                {isSuccess ? (
                  ...
                  >
                    ...
                    <span className="text-sm font-mono uppercase font-bold tracking-widest text-neon-emerald">
                      Access Granted
                    </span>
                    <span className="text-[9px] font-mono text-foreground/50">
                      {geminiResult?.type === "ticket"
                        ? "QR Matchday Pass Verified"
                  ...
                  isScanning && (
                    ...
                    >
                      <span className="text-[9px] font-mono uppercase tracking-widest bg-black/60 px-2 py-0.5 rounded">
                        Scan Complete
                      </span>
                      <button
                        onClick={handleStartScan}
              ...
              <div className="flex flex-col items-center gap-4 text-center p-6">
                <div className="w-14 h-14 rounded-full bg-neon-pink/5 border border-neon-pink/15 flex items-center justify-center relative">
                  ...
                  <div
                    className="absolute inset-0 rounded-full border border-neon-pink/10 animate-ping"
          ...
          <div className="bg-black/90 border border-white/5 p-3 rounded-sm font-mono text-[9px] text-foreground/70 h-32 overflow-y-auto flex flex-col gap-1 leading-relaxed relative">
            <div className="absolute top-2 right-2 flex items-center gap-1.5">
              ...
              <span className="text-[7px] text-foreground/25 uppercase">
                Terminal
              </span>
            </div>
            {scanLog.map((log, i) => (
            ...
            {[
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="text-xs font-medium text-foreground group-hover:text-neon-orange/90 transition-colors">
                      {item.name}
                    </h5>
                    <div className="flex gap-2 text-[9px] font-mono text-foreground/40 mt-0.5">
                      <span className="text-neon-orange/70 font-semibold">
                        {item.price}
                      </span>
                      <span>·</span>
        ...
        >
          ...
          <p className="text-[11px] text-foreground/55 leading-relaxed font-light">
            Central telemetry log showing real-time inflow pressure, entrance
            queue status, and facility operations from IoT stadium sensors.
          </p>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div
                key={stat.label}
                className="bg-black/40 border border-white/5 rounded-sm p-2 text-center"
              >
                <span
                  className={`block text-lg font-mono font-bold ${stat.color}`}
                >
                  {stat.value}
                </span>
                <span className="text-[7px] font-mono text-foreground/30 uppercase tracking-wider">
                  {stat.label}
                </span>
              </div>
            ))}
