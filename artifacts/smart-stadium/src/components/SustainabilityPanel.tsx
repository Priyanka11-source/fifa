...
export default function SustainabilityPanel({ energyLoad = 64, activeIncident = 'none' }: SustainabilityPanelProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-10">
      {/* Header */}
      <div>
        <span className="text-[10px] font-mono text-neon-emerald uppercase tracking-[0.2em] mb-1 block">Green Telemetry</span>
        <h3 className="font-serif text-3xl text-foreground">Sustainability & Energy Dashboard</h3>
        <p className="text-xs text-foreground/60 mt-1 font-light leading-relaxed">
          Tracking the environmental footprint of the FIFA World Cup 2026. Powered by micro-generation solar loops and wastewater recycling.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        ...
        <div className="glass-panel p-6 rounded-sm flex flex-col gap-4 border-t-2 border-t-neon-gold shadow-neon-gold shadow-neon-gold-hover hover:bg-white/5">
          ...
          <div>
            <span className="text-[10px] font-mono text-foreground/45 uppercase tracking-wider block mb-0.5">Solar Output</span>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-serif font-bold text-neon-gold">{currentSolarGeneration}</span>
          ...
          <div className="text-[11px] text-foreground/50 font-light border-t border-primary/5 pt-3 leading-relaxed">
            Supplying {isStorm ? '5%' : '34%'} of active stadium electrical demand. {isStorm ? 'Cloud cover throttling collectors.' : 'Optimum absorption index.'}
          </div>
        </div>
        ...
        <div className="glass-panel p-6 rounded-sm flex flex-col gap-4 border-t-2 border-t-neon-emerald shadow-neon-emerald shadow-neon-emerald-hover hover:bg-white/5">
          ...
          <div>
            <span className="text-[10px] font-mono text-foreground/45 uppercase tracking-wider block mb-0.5">Battery Bank Storage</span>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-serif font-bold text-neon-emerald">{batteryReserveLevel}</span>
              <span className="text-xs font-mono text-foreground/80">% capacity</span>
            </div>
          </div>
          <div className="text-[11px] text-foreground/50 font-light border-t border-primary/5 pt-3 leading-relaxed">
            {isGridFailure 
        ...
        <div className="glass-panel p-6 rounded-sm flex flex-col gap-4 border-t-2 border-t-neon-cyan shadow-neon-cyan shadow-neon-cyan-hover hover:bg-white/5">
          ...
          <div>
            <span className="text-[10px] font-mono text-foreground/45 uppercase tracking-wider block mb-0.5">Rainwater Collector</span>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-serif font-bold text-neon-cyan">
                {(rainwaterReclaimed / 1000).toFixed(1)}k
              </span>
              <span className="text-xs font-mono text-foreground/80">Liters</span>
            </div>
          </div>
          <div className="text-[11px] text-foreground/50 font-light border-t border-primary/5 pt-3 leading-relaxed">
            {isStorm 
      ...
      <div className="glass-panel p-6 rounded-sm flex flex-col gap-6 relative overflow-hidden">
        <div>
          <span className="text-[10px] font-mono text-neon-emerald uppercase tracking-[0.2em] mb-1 block">Live Load balance</span>
          <h4 className="text-lg font-medium text-foreground">Peak Demand vs. Solar Generation (MW)</h4>
        </div>
