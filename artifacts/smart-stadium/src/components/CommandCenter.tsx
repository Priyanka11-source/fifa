...
const SEVERITY_STYLE: Record<string, string> = {
  critical:
    "bg-indigo-500/20 text-indigo-400 border-indigo-500/40 warn-neon-glow",
...
export default function CommandCenter() {
  ...
  const {
    ...
    data: brief,
    isPending: isBriefPending,
  } = useGenerateOperationsBrief();

  const { mutate: simulateIncident, isPending: isSimulating } =
    useSimulateIncident();
    ...
    updateCustomTelemetry(
      {
        data: {
          ...
          gates: Object.entries(customGates).map(([id, crowdPct]) => ({
            id,
            crowdPct: Number(crowdPct),
          })),
        },
      },
      {
  ...
  function handleSimulate(
    type: "storm" | "transit_disruption" | "crowd_surge" | "grid_failure",
  ) {
    setActiveAlert(type);
    simulateIncident(
      { data: { type } },
      ...
      <div className="glass-panel p-6 rounded-sm border-l-2 border-l-primary/40 relative overflow-hidden flex flex-col xl:flex-row gap-8 justify-between items-start xl:items-center">
        ...
        <div className="flex flex-col gap-1 z-10 max-w-2xl">
          ...
          <h4 className="text-xl font-serif text-foreground">
            Real-Time GenAI Decision Support Tool
          </h4>
          <p className="text-xs text-foreground/75 font-light leading-relaxed">
            Simulate emergency matchday incidents. The platform will dynamically
            calculate response directives, dispatch virtual safety stewards, and
            reroute fan flows on the maps and signages.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 z-10 shrink-0">
          {(
            ...
            return (
              ...
              >
                <Icon
                  className={`w-4 h-4 ${isActive ? "animate-bounce text-indigo-500" : ""}`}
                />
          ...
          >
            <div className="flex items-center gap-2 border-b border-primary/10 pb-2">
              ...
              <span className="font-mono text-xs text-primary uppercase tracking-widest">
                Real-Time Telemetry Data Entry
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="flex flex-col justify-center text-[10px] text-foreground/40 leading-relaxed font-mono border-l border-primary/10 pl-6">
                <span>
                  * Entering custom telemetry switches the system to MANUAL
                  Incident Command Mode. Ticks are frozen to preserve your
                  inputs.
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <label className="text-[10px] font-mono uppercase tracking-wider text-foreground/60">
                Gate Crowd Density (Crowd %)
              </label>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                {state?.gates.map((g) => (
                  <div
                    key={g.id}
                    className="bg-card/40 border border-primary/10 p-3 rounded-sm flex flex-col gap-2"
                  >
                    <span
                      className="text-[8px] font-mono text-foreground/50 uppercase truncate"
                      title={g.name}
                    >
                      {g.name.split(" — ")[0]}
                    </span>
            ...
            <AnimatePresence mode="popLayout">
              {directives.map((directive) => {
                ...
                if (directive.severity === "critical")
                  borderClass = "border-indigo-500/30 warn-neon-glow";
                if (directive.severity === "watch")
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-[8.5px] uppercase font-mono tracking-wider px-2 py-0.5 rounded-sm border ${SEVERITY_STYLE[directive.severity]}`}
                      >
                        {directive.status}
                      </span>
                    </div>
        ...
        <div className="w-full lg:w-[360px] glass-panel p-6 rounded-sm flex flex-col justify-between shrink-0 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-5 h-full justify-between">
              ...
              {state && (
                <div className="grid grid-cols-2 gap-3 mb-6 bg-white/5 border border-primary/10 p-3.5 rounded-sm font-mono text-[10px] text-foreground/60">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span
                      className={`font-bold ${state.activeIncident !== "none" ? "text-red-400" : "text-emerald-400"}`}
                    >
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-foreground font-medium">
                      {state.crowdCount.toLocaleString()}
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="mt-4 pt-4 border-t border-primary/15 flex flex-col gap-2">
              <span className="text-[10px] font-mono text-foreground/40 uppercase tracking-widest block">
                Executive Summary
              </span>
