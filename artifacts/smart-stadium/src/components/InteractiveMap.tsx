...
import { Info, MapPin, CheckCircle, AlertTriangle, Battery, Compass, ShieldAlert, Award } from 'lucide-react';
...
type MapLayer = 'crowd' | 'transit' | 'accessibility' | 'sustainability' | 'match3d';
...
function Match3DGame() {
  ...
  useEffect(() => {
    ...
    let players: Player[] = [
      { x: -210, y: 0, vx: 0, vy: 0, team: 'blue', role: 'keeper', number: 1, name: 'AI Keeper' },
      { x: -90, y: -40, vx: 0, vy: 0, team: 'blue', role: 'midfielder', number: 8, name: 'AI Mid' },
      { x: -30, y: 30, vx: 0, vy: 0, team: 'blue', role: 'striker', number: 10, name: 'You (Striker)' },
      
      { x: 210, y: 0, vx: 0, vy: 0, team: 'red', role: 'keeper', number: 1, name: 'Red Keeper' },
      { x: 90, y: 40, vx: 0, vy: 0, team: 'red', role: 'midfielder', number: 6, name: 'Red Mid' },
      { x: 30, y: -30, vx: 0, vy: 0, team: 'red', role: 'striker', number: 9, name: 'Red Striker' }
    ];
    ...
    function resetPositions() {
      ...
      players[0].x = -210; players[0].y = 0;
      players[1].x = -90;  players[1].y = -40;
      players[2].x = -30;  players[2].y = 30;
      players[3].x = 210;  players[3].y = 0;
      players[4].x = 90;   players[4].y = 40;
      players[5].x = 30;   players[5].y = -30;
    ...
    function update() {
      ...
      user.vx = 0;
      user.vy = 0;
      if (keysPressed.current['ArrowUp'] || keysPressed.current['w']) user.vy = -speed;
      if (keysPressed.current['ArrowDown'] || keysPressed.current['s']) user.vy = speed;
      if (keysPressed.current['ArrowLeft'] || keysPressed.current['a']) user.vx = -speed;
      if (keysPressed.current['ArrowRight'] || keysPressed.current['d']) user.vx = speed;
        ...
        if (dist > 10) {
          ...
          if (ball.z < 12) {
            if (p.team === 'red') {
              ...
              const angle = Math.atan2(players[2].y - ball.y, players[2].x - ball.x);
    ...
    function draw() {
      ...
      ctx!.beginPath();
      ctx!.ellipse(bShadow.x, bShadow.y, shadowRadius * 1.5, shadowRadius * 0.8, 0, 0, Math.PI * 2);
      ...
      const entities = [
        ...players.map((p, idx) => ({ type: 'player', y: p.y, data: p, key: idx })),
    ...
    <div className="w-full h-full flex flex-col gap-4 relative">
      <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-black/85 border border-[#f5b041]/40 px-6 py-2.5 rounded-sm flex items-center justify-between gap-6 z-30 font-mono shadow-[0_0_15px_rgba(245,176,65,0.15)]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-foreground/80 uppercase font-bold tracking-widest">BLUE TEAM</span>
        </div>
        <span className="text-xl text-[#f5b041] font-bold">{score.blue} - {score.red}</span>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-foreground/80 uppercase font-bold tracking-widest">RED TEAM</span>
      ...
      {goalText && (
        <div className="absolute inset-0 bg-black/55 flex items-center justify-center z-40 animate-pulse">
          <div className="text-center">
            ...
            <p className="text-[10px] font-mono text-white/60 tracking-wider">RESETTING BALL POSITIONS...</p>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 bg-black/60 rounded-sm overflow-hidden min-h-[440px] relative border border-white/5 shadow-2xl">
        ...
        <div className="absolute bottom-4 left-4 bg-black/85 border border-white/10 px-4 py-2.5 rounded-sm font-mono text-[9px] text-foreground/70 flex flex-col gap-1 z-30 leading-relaxed shadow-lg">
          <div className="flex gap-2">
            <span className="text-[#00f5d4] font-bold">🎮 INTERACTIVE 3D PLAYGROUND</span>
          </div>
          <div>· Move Blue Striker (10): <span className="text-white font-semibold">Arrow Keys</span> or <span className="text-white font-semibold">WASD</span></div>
          <div>· Shoot / Kick ball: <span className="text-white font-semibold">Spacebar</span></div>
          <div className="text-[8px] text-[#f5b041] mt-1 font-semibold animate-pulse">👉 KICK THE BALL INTO THE RED GOAL!</div>
        </div>
      </div>
    </div>
  );
...
export default function InteractiveMap({ gates = [], transport = [], weather = 'Clear', activeIncident = 'none' }: InteractiveMapProps) {
  ...
  const hotspots: Hotspot[] = [
    ...
    },
    {
      ...
      status: activeIncident === 'crowd_surge' ? 'critical' : activeIncident === 'storm' ? 'warning' : 'warning',
      aiRec: activeIncident === 'crowd_surge' 
    ...
    },
    {
      ...
      status: 'nominal',
      aiRec: 'Nominal operations. Displaying family mascot greetings on screens.',
    },
    {
      id: 'gate-9',
    ...
    },
    {
      ...
      y: 350,
      layers: ['crowd', 'transit'],
      info: activeIncident === 'crowd_surge'
    ...
    },
    {
      ...
      y: 120,
      layers: ['transit'],
      info: activeIncident === 'transit_disruption'
      ...
      aiRec: activeIncident === 'transit_disruption'
    ...
    },
    {
      ...
      info: activeIncident === 'grid_failure'
    ...
    },
    {
      ...
      status: 'nominal',
      aiRec: 'Elevator sensors report 100% uptime. Braille and voice guides functional.',
    }
  ];

  const filteredHotspots = hotspots.filter(h => h.layers.includes(activeLayer));
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-8 items-stretch">
      ...
      <div className="w-full lg:w-[320px] flex flex-col gap-6 shrink-0">
        <div>
          <span className="text-[10px] font-mono text-secondary uppercase tracking-[0.2em] mb-1 block font-semibold">Telemetry Data Layer</span>
          <h3 className="font-serif text-2xl text-foreground">Spatial Analysis Mapping</h3>
          <p className="text-xs text-foreground/60 mt-1 font-light leading-relaxed">
            Select telemetry overlays to analyze real-time spatial logistics, egress channels, and GenAI resource dispatch coordinates.
          </p>
        </div>

        {/* Tab Controls */}
        <div className="flex flex-col gap-2.5">
          {(['crowd', 'transit', 'accessibility', 'sustainability', 'match3d'] as MapLayer[]).map((layer) => (
            <button
              key={layer}
              onClick={() => {
            ...
            >
              ...
              <div className={`w-1.5 h-1.5 rounded-full ${activeLayer === layer ? 'bg-secondary animate-pulse' : 'bg-foreground/20'}`} />
            </button>
          ))}
        </div>
        ...
        <div className="border-t border-white/10 pt-4 flex flex-col gap-2">
          <span className="text-[10px] font-mono text-foreground/45 uppercase tracking-[0.2em] mb-1 block font-medium">Logistics Simulation</span>
          <button
          ...
          >
            <span className="text-[11px] uppercase tracking-wider font-mono">🏃 Egress Flow Simulation</span>
            <span className={`w-2 h-2 rounded-full ${egressSimActive ? 'bg-secondary animate-ping' : 'bg-foreground/20'}`} />
          </button>
        </div>
            ...
            {selectedHotspot ? (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <h4 className="text-sm font-medium text-foreground">{selectedHotspot.name}</h4>
                ...
                <p className="text-xs text-foreground/80 font-light leading-relaxed">{selectedHotspot.info}</p>
              ...
              >
                ...
                <span className="text-xs text-foreground/40 font-mono uppercase tracking-wider">Select coordinate node</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        {activeLayer === 'match3d' ? (
          ...
          <>
            ...
            >
              ...
              {/* Outer glow aura */}
              <ellipse cx="350" cy="260" rx="300" ry="210" fill="url(#stadiumGlow)" />
                ...
                {activeLayer === 'transit' && (
                  ...
                  >
                    ...
                    <path 
                      ...
                      className={activeIncident === 'transit_disruption' ? 'text-red-500 stroke-red-500' : ''}
                    />
                    {/* Shuttle Bus Loop */}
                    <rect 
                      x="70" 
                    ...
                    <circle cx="200" cy="60" r="4" fill="var(--color-secondary)" className="animate-pulse" />
                    <circle cx="500" cy="440" r="4" fill="var(--color-secondary)" className="animate-pulse" />
                  </motion.g>
                )}
              </AnimatePresence>
              ...
              <AnimatePresence>
                {activeLayer === 'accessibility' && (
                  ...
                  >
                    ...
                    <path d="M 120,280 L 160,260 L 220,430" stroke="rgba(46, 204, 113, 0.4)" strokeWidth="3" fill="none" strokeDasharray="5 5" />
                    <path d="M 580,180 L 520,230 L 480,350" stroke="rgba(46, 204, 113, 0.4)" strokeWidth="3" fill="none" strokeDasharray="5 5" />
                  </motion.g>
                )}
              </AnimatePresence>
              ...
              <line x1="350" y1="140" x2="350" y2="90" stroke="rgba(255, 255, 255, 0.12)" strokeWidth="1" />
              <line x1="350" y1="380" x2="350" y2="430" stroke="rgba(255, 255, 255, 0.12)" strokeWidth="1" />
              <line x1="170" y1="260" x2="110" y2="260" stroke="rgba(255, 255, 255, 0.12)" strokeWidth="1" />
              <line x1="530" y1="260" x2="590" y2="260" stroke="rgba(255, 255, 255, 0.12)" strokeWidth="1" />
              ...
              {/* Field Glow Area */}
              <ellipse cx="350" cy="260" rx="90" ry="55" fill="url(#fieldGlow)" />
              ...
              {activeLayer === 'crowd' && (
                <g className="pointer-events-none">
                  ...
                  <circle cx="120" cy="280" r="45" fill="url(#heatCritical)" className="heat-blob" />
                  <circle cx="480" cy="350" r="40" fill="url(#heatWarning)" className="heat-blob" style={{ animationDelay: '1s' }} />
                  {/* Heat blobs on the pitch itself */}
                  <circle cx="350" cy="260" r="50" fill="url(#heatCritical)" className="heat-blob" style={{ animationDelay: '0.5s' }} />
                  <circle cx={350 + 40 * 0.866} cy={260 + 40 * 0.5 * 0.65} r="35" fill="url(#heatWarning)" className="heat-blob" style={{ animationDelay: '1.5s' }} />
                  <circle cx={350 - 40 * 0.866} cy={260 - 40 * 0.5 * 0.65} r="35" fill="url(#heatWarning)" className="heat-blob" style={{ animationDelay: '2.5s' }} />
                </g>
              )}
              ...
              <AnimatePresence>
                {activeLayer === 'sustainability' && (
                  ...
                  >
                    ...
                    <path d="M 230,110 A 150,90 0 0,1 470,110" fill="none" stroke="rgba(255, 145, 0, 0.4)" strokeWidth="6" strokeDasharray="3 3" />
                    <path d="M 230,410 A 150,90 0 0,0 470,410" fill="none" stroke="rgba(255, 145, 0, 0.4)" strokeWidth="6" strokeDasharray="3 3" />
                  </motion.g>
                )}
              </AnimatePresence>
              ...
              {filteredHotspots.map((hotspot) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(hotspot.status === 'critical' || hotspot.status === 'warning' || isSelected) && (
                      <circle 
                        cx="0" 
                        cy="0" 
                        r={isSelected ? "16" : "12"} 
                        fill="none" 
                        stroke={hotspot.status === 'critical' ? '#ff1744' : hotspot.status === 'warning' ? '#ff9100' : 'var(--color-primary)'}
                        strokeWidth="1.5"
                    ...
                    <circle 
                      ...
                      r={isSelected ? "9" : "7"} 
                      fill="#020403" 
                      stroke={hotspot.status === 'critical' ? '#ff1744' : hotspot.status === 'warning' ? '#ff9100' : activeLayer === 'accessibility' ? 'var(--color-secondary)' : 'var(--color-primary)'}
