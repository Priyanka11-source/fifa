...
import { Train, Bus, Car, ParkingSquare, AlertCircle, Clock, CheckCircle } from 'lucide-react';
...
export default function TransitBoard({ transport = [], activeIncident = 'none' }: TransitBoardProps) {
  return (
    <div className="w-full flex flex-col gap-8">
      {/* Board Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-[10px] font-mono text-neon-orange uppercase tracking-[0.2em] mb-1 block">Live Departures</span>
          <h3 className="font-serif text-3xl text-foreground">Transit & Mobility Board</h3>
          <p className="text-xs text-foreground/60 mt-1 font-light leading-relaxed">
            Live telemetry from central transit controls. Plan your post-match egress routes using GenAI coordination.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {transport.map((line) => {
          const Icon = TRANSIT_ICONS[line.mode as keyof typeof TRANSIT_ICONS] || Bus;
          const label = MODE_LABELS[line.mode as keyof typeof MODE_LABELS] || 'Transit';
          
          let cardStyle = 'border-t-neon-cyan shadow-neon-cyan shadow-neon-cyan-hover';
          let statusColor = 'text-neon-cyan bg-neon-cyan/10 border-neon-cyan/20';
          let iconColor = 'text-neon-cyan';

          if (line.status === 'disrupted') {
            cardStyle = 'border-t-neon-pink shadow-neon-pink shadow-neon-pink-hover';
            statusColor = 'text-neon-pink bg-neon-pink/10 border-neon-pink/20 animate-pulse';
            iconColor = 'text-neon-pink';
          } else if (line.status === 'delayed') {
            cardStyle = 'border-t-neon-orange shadow-neon-orange shadow-neon-orange-hover';
            statusColor = 'text-neon-orange bg-neon-orange/10 border-neon-orange/20';
            iconColor = 'text-neon-orange';
          } else {
            cardStyle = 'border-t-neon-emerald shadow-neon-emerald shadow-neon-emerald-hover';
            statusColor = 'text-neon-emerald bg-neon-emerald/10 border-neon-emerald/20';
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start">
                <div className={`w-10 h-10 border border-white/10 rounded flex items-center justify-center bg-black/40 ${iconColor} shadow-[0_0_10px_rgba(255,255,255,0.02)]`}>
                  <Icon className="w-5 h-5" />
                </div>
                <span className={`text-[9px] uppercase font-mono px-2 py-0.5 rounded border ${statusColor}`}>
                  {line.status}
                </span>
              </div>

              <div>
                <span className="text-[10px] font-mono text-foreground/45 uppercase tracking-wider block mb-0.5">{label}</span>
                <h4 className="text-sm font-bold text-foreground">{line.name}</h4>
              </div>

              <div className="flex items-baseline gap-2.5 mt-2 border-t border-primary/5 pt-4">
                {line.mode === 'parking' ? (
                  <>
                    <span className={`text-3xl font-serif font-bold ${iconColor}`}>84</span>
                    <span className="text-[10px] font-mono text-foreground/40 uppercase">% occupied</span>
                  </>
                ) : (
                  <>
                    <span className={`text-3xl font-serif font-bold ${iconColor}`}>
                      {line.etaMinutes === 0 ? 'Boarding' : line.etaMinutes}
                    </span>
                    {line.etaMinutes > 0 && (
                      <span className="text-[10px] font-mono text-foreground/40 uppercase">mins</span>
                    )}
                  </>
                )}
              </div>
              ...
              <div className="mt-auto border-t border-primary/5 pt-3 flex gap-2 items-start text-[11px] text-foreground/50 font-light leading-relaxed">
                ...
                <span>
                  {line.status === 'disrupted' && 'AI: Take Shuttle Loop A to intercept train further down the line.'}
                  {line.status === 'delayed' && 'AI: Flow congestion expected. Consider boarding 5 mins early.'}
                  {line.status === 'normal' && 'AI: Boarding intervals optimal. Direct pathways clear.'}
                </span>
              </div>
            </div>
          );
      ...
      <div className="p-5 border-t-2 border-t-neon-purple bg-gradient-to-r from-neon-purple/10 via-[#0c081e] to-neon-pink/5 rounded-sm flex flex-col md:flex-row items-start md:items-center gap-4 relative overflow-hidden shadow-neon-purple shadow-neon-purple-hover">
        ...
        <div className="flex-1 flex flex-col gap-1 z-10">
          <h5 className="text-xs uppercase font-mono text-neon-purple tracking-widest font-bold">GenAI Mobility Copilot Recommendation</h5>
